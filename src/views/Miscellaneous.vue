<template>
  <section class="static">
    ...
    <ckeditor
        :editor="editor"
        v-model="staticInfo.contentMiscellaneous"
        :config="editorConfig"
        @ready="onReady"
    ...
    <div class="new-editor" v-if="staticInfo && !isEditorActive" v-html="staticInfo.contentMiscellaneous">
...
const getStatic = async () => {
  await axios.get('static-pages')
      .then((staticData) => {
        ...
        console.log(staticInfo.value.contentMiscellaneous)
      })
}
...
const saveStatic = async () => {
  await axios.put('static-pages/1', {
    id: 1,
    contentMiscellaneous: staticInfo.value.contentMiscellaneous
  })
      .then(() => {
        location.reload()
      })
