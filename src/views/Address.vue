<template>
  <section class="static">
    ...
    <ckeditor
        :editor="editor"
        v-model="staticInfo.contentContacts"
        :config="editorConfig"
        @ready="onReady"
    ...
    <div class="new-editor" v-if="staticInfo && !isEditorActive" v-html="staticInfo.contentContacts">
...
const getStatic = async () => {
  await axios.get('static-pages')
      .then((staticData) => {
        ...
        console.log(staticInfo.value.contentContacts)
      })
}
...
const saveStatic = async () => {
  await axios.put('static-page/1', {
    id: 1,
    contentContacts: staticInfo.value.contentContacts
  })
      .then(() => {
        location.reload()
      })
