  ...
  <div class="custom-table">
    ...
    <GDialog v-model="studentActionDialogState" :max-width="400">
      <div class="delete-confirm-modal">
        <p>
          {{ studentActionType === 'disable' ? 'Скрыть студента' : 'Полностью удалить студента' }}
          <strong>{{ studentActionTarget?.lastName }} {{ studentActionTarget?.firstName }} {{ studentActionTarget?.patronymic }}</strong>?
        </p>
        <div class="delete-confirm-modal__actions">
          <button @click="submitStudentAction" class="delete-confirm-modal__delete">
...
const getStudents = async () => {
  await axios.get('students')
      .then((userData) => {
        studentsList.value = userData.data.filter(student => student.isActive !== false)
      })
}
...
async function submitStudentAction() {
  ...
  try {
    ...
    alert(studentActionType.value === 'disable'
