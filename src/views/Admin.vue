...
const newsUrl = ref();
const admRole = ref();
const hiddenEmployeesStorageKey = "adminHiddenEmployees";

const readHiddenEmployees = () => {
  try {
    return JSON.parse(localStorage.getItem(hiddenEmployeesStorageKey) || "[]");
  } catch (error) {
    return [];
  }
};

const saveHiddenEmployees = (employees) => {
  localStorage.setItem(hiddenEmployeesStorageKey, JSON.stringify(employees));
};

const cacheHiddenEmployee = (employee) => {
  const hiddenEmployees = readHiddenEmployees().filter(
    (item) => item.id !== employee.id,
  );
  hiddenEmployees.push({ ...employee, isActive: false });
  saveHiddenEmployees(hiddenEmployees);
};

const removeHiddenEmployee = (employeeId) => {
  saveHiddenEmployees(
    readHiddenEmployees().filter((employee) => employee.id !== employeeId),
  );
};

const mergeHiddenEmployees = (employees) => {
  const employeeMap = new Map(
    employees.map((employee) => [employee.id, employee]),
  );
  readHiddenEmployees().forEach((employee) => {
    if (!employeeMap.has(employee.id)) {
      employeeMap.set(employee.id, { ...employee, isActive: false });
    }
  });
...
const getUsers = async () => {
  await axios.get("employees").then((userData) => {
    userList.value = mergeHiddenEmployees(userData.data);
  });
};
...
const getEmployeeVisibilityActionLabel = (user) => {
  return user?.isActive === false ? "Показать" : "Скрыть";
};

const getEmployeeVisibilityDialogTitle = (user) => {
  return user?.isActive === false ? "Показать сотрудника" : "Скрыть сотрудника";
};
...
const submitEmployeeAction = async () => {
  ...
  try {
    if (employeeActionType.value === "disable") {
      const employee = employeeActionTarget.value;
      const nextIsActive = employee.isActive === false;
      await axios.patch(`employees/${employee.id}/disable`, {});

      if (nextIsActive) {
        removeHiddenEmployee(employee.id);
        const activeEmployee = { ...employee, isActive: true };
        userList.value = userList.value.map((user) =>
          user.id === employee.id ? activeEmployee : user,
        );
      } else {
        const inactiveEmployee = { ...employee, isActive: false };
        cacheHiddenEmployee(inactiveEmployee);
        userList.value = userList.value.map((user) =>
          user.id === employee.id ? inactiveEmployee : user,
        );
      }
    } else {
      await axios.delete(`employees/${employeeActionTarget.value.id}`);
      removeHiddenEmployee(employeeActionTarget.value.id);
      userList.value = userList.value.filter(
        (user) => user.id !== employeeActionTarget.value.id,
      );
    }
    ...
    alert(
      employeeActionType.value === "disable"
        ? `Ошибка при попытке ${getEmployeeVisibilityActionLabel(
            employeeActionTarget.value,
          ).toLowerCase()} сотрудника`
...
<template>
    ...
    <div class="admin__view">
          ...
          >
            ...
            >
              ...
              <textarea
                ...
                class="slider-admin__item-text"
                :disabled="checkDisable[index]"
                placeholder="Введите описание для слайда"
              ></textarea>
              <input
                :disabled="checkDisable[index]"
                type="text"
      ...
      <div v-show="activeItem === 'feed'" class="admin__view-item">
        <router-link
          ...
          class="admin-button news-create-btn"
        >
          <svg
            class="news-create-btn__icon"
            width="18"
            height="18"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="news-card__cover">
              <img
                ...
                :class="[
                  ...
                  {
                    'news-card__cover-img--placeholder': isNewsPlaceholder(
                      newsSlide.imageLink,
                    ),
                  },
                ]"
              ...
              <label
                ...
                :for="newsSlide.id"
                class="news-card__cover-overlay"
              >
                <svg
                  width="28"
                  height="28"
      ...
      <div v-show="activeItem === 'schedule'" class="admin__view-item">
        ...
        <div v-if="weekConfigLoaded" class="week-config">
          <div class="week-config__summary" v-if="!weekConfigEditing">
            <span>
              Сейчас идёт
              <b>{{ currentIsDenominator ? "знаменатель" : "числитель" }}</b
              >. Опорная неделя —
              {{ formatReferenceDate(weekConfigForm.reference_date) }} ({{
                weekConfigForm.is_denominator ? "знаменатель" : "числитель"
              }}).
            </span>
            <button class="week-config__link" @click="weekConfigEditing = true">
              Изменить
            </button>
          </div>
          ...
          <div class="week-config__editor" v-else>
            <div class="week-config__row">
              <label class="week-config__field">
                ...
                <span
                  v-if="
                    weekConfigForm.reference_date &&
                    referenceDateIsMonday === false
                  "
            ...
            <div class="week-config__row">
              <div
                class="week-config__toggle"
                role="radiogroup"
                aria-label="Тип опорной недели"
              >
                <button
                  type="button"
                  role="radio"
          ...
          <div v-if="weekConfigError" class="schedule-error">
            {{ weekConfigError }}
          </div>
          <div v-if="weekConfigSuccess" class="week-config__success">
            {{ weekConfigSuccess }}
          </div>
        </div>
      </div>
        ...
        <div class="admin-users">
          <div class="admin-users__item">
            ...
            <div style="display: flex; gap: 10px">
              ...
              >
                <option value="">Как с сервера</option>
            ...
            <div class="admin-users__item-list">
              <div
                ...
                :class="[
                  'user-item',
                  { 'user-item--inactive': user.isActive === false },
                ]"
              >
                ...
                <template v-if="activeEmployeesTab === 'employees'">
                  <div class="employee-actions">
                    ...
                    <button
                      ...
                      :aria-label="getEmployeeVisibilityActionLabel(user)"
                      :title="`${getEmployeeVisibilityActionLabel(
                        user,
                      )} сотрудника`"
  ...
  <GDialog v-model="employeeActionDialogState" :max-width="400">
    <div class="delete-confirm-modal">
      <p>
        {{
          employeeActionType === "disable"
            ? getEmployeeVisibilityDialogTitle(employeeActionTarget)
            : "Полностью удалить сотрудника"
        }}
        <strong
          >{{ employeeActionTarget?.lastName }}
          {{ employeeActionTarget?.firstName }}
          {{ employeeActionTarget?.patronymic }}</strong
        >?
      </p>
      <div class="delete-confirm-modal__actions">
        <button
          @click="submitEmployeeAction"
          class="admin-button delete-confirm-modal__delete"
        >
          {{
            employeeActionType === "disable"
              ? getEmployeeVisibilityActionLabel(employeeActionTarget)
              : "Удалить"
          }}
        </button>
        <button @click="closeEmployeeActionDialog" class="admin-button">
          Отмена
        </button>
      </div>
    </div>
  ...
  <GDialog v-model="deleteNewsDialogState" :max-width="420">
    <div class="delete-confirm-modal">
      <p>
        Удалить новость
        <strong v-if="newsToDelete?.title">«{{ newsToDelete.title }}»</strong
        ><span v-else>без заголовка</span>?
      </p>
      <div class="delete-confirm-modal__actions">
        <button
        ...
        <button @click="deleteNewsDialogState = false" class="admin-button">
          Отмена
        </button>
      </div>
    </div>
